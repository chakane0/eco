...
*For any* market that exists in the database before a sync operation with `currentPrice = X`, after the sync completes, `previousPrice` SHALL equal `X` and `currentPrice` SHALL equal the new value from Kalshi. For any market fetched for the first time, `previousPrice` SHALL equal `currentPrice`.

**Validates: Requirements 1.2, 1.3**

### Property 2: Market Categorization Correctness

*For any* market title, `categorizeMarket` SHALL return the same Market_Category (or null) on every invocation with that title. If the title contains a keyword from a category's keyword list, the function SHALL return that category. If the title contains no keywords from any category, the function SHALL return null.

**Validates: Requirements 2.1, 2.2, 2.3**

### Property 3: Top Market Selection Correctness

*For any* array of markets and any positive integer limit, `selectTopMarkets` SHALL return at most `limit` markets sorted by volume descending, and every included market SHALL have volume greater than or equal to every excluded market.

**Validates: Requirements 3.1, 3.2, 3.3**

### Property 4: Insight Prompt Completeness

*For any* valid MarketSnapshot, the structured prompt built by the Insight_Generator SHALL contain the market title, current probability, trend direction and magnitude, and category.

**Validates: Requirement 4.2**

### Property 5: Insight Upsert Uniqueness

*For any* market, after generating insights multiple times for that market, exactly one active insight record SHALL exist in the database for that market.

**Validates: Requirement 4.4**

### Property 6: Market Data Validation

*For any* market stored in the database: `currentPrice` and `previousPrice` SHALL be within [0, 1] inclusive, `volume` SHALL be a non-negative integer, and `title` SHALL be a non-empty string of at most 500 characters.

**Validates: Requirements 5.1, 5.2, 5.4**

### Property 7: Trend Calculation Consistency

*For any* market with a (currentPrice, previousPrice) pair: if `currentPrice > previousPrice` then trend SHALL be "up"; if `currentPrice < previousPrice` then trend SHALL be "down"; if `currentPrice = previousPrice` then trend SHALL be "stable". The `trendPercent` SHALL equal `(currentPrice - previousPrice) * 100`.

**Validates: Requirements 6.1, 6.2, 6.3, 6.4**

### Property 8: Subscriber Idempotency

*For any* valid email address, calling `addSubscriber` with that email multiple times SHALL result in exactly one subscriber record with that email and `active = true`.

**Validates: Requirements 7.1, 7.2, 7.5**

### Property 9: Subscriber Reactivation

*For any* email that was subscribed and then deactivated, calling `addSubscriber` again SHALL set `active = true` without creating a duplicate record.

**Validates: Requirement 7.3**

### Property 10: Invalid Email Rejection

*For any* string that is not a valid email format (including empty strings), `addSubscriber` SHALL reject the input and store nothing in the database.

**Validates: Requirement 7.4**

### Property 11: Digest Mover Selection

*For any* set of markets in the database, the digest SHALL select at most 5 markets sorted by absolute price change (`|currentPrice - previousPrice|`) in descending order, and every selected market SHALL have an absolute price change greater than or equal to every non-selected market.

**Validates: Requirement 9.1**

### Property 12: Digest Delivery Accounting

*For any* digest send operation with result `r` and `n` active subscribers: `r.sent + r.failed` SHALL equal `n`.

**Validates: Requirement 9.6**

### Property 13: Market Card Rendering Completeness

*For any* MarketWithInsight object with a non-null insight, the rendered market card SHALL contain the market title, probability as a percentage, trend direction arrow, trend magnitude percentage, and the insight text.

**Validates: Requirement 10.2**

### Property 14: AI Output Sanitization

*For any* string containing HTML tags or script elements, the sanitization function SHALL remove or escape dangerous content before including it in API responses.
